# 使用 `System.Data.Odbc` 与 `Access` 数据库进行交互

若项目是基于 `.NET Framework`，请使用 `OleDb`，详情参阅 [C#——使用OleDb操作Access数据库(.NET Framework)](https://blog.csdn.net/qq_39395589/article/details/88701428)

前往 `NuGet` 添加 `System.Data.Odbc` 包的引用，也可直接编辑项目的csproj文件，在 `ItemGroup` 节点添加下述引用
...
namespace SomeNameSpace
{
    ...
    {
        // 定义连接字符串
        public static string AccessFileName { get; set; }
        public static readonly string ConnString =
            @"Driver={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=" +
            AppDomain.CurrentDomain.BaseDirectory +
            @$"Files\{AccessFileName}.accdb;";
        ...
        public static bool IsTableExist(string tableName)
        {
            ...
            {
                System.Windows.MessageBox.Show($"数据库中不存在表: {tableName}");
                return false;
            }
        }
